...
load(fullfile(getenv('OBSDATADIR'), 'sessions', session, 'runAnalyzed.mat'), ...
        'obsOnTimes', 'obsOffTimes',  'wiskContactFrames', 'frameTimeStamps', 'rewardTimes', 'isLightOn');
...
for cell = cells
    ...
    fig = figure('color', 'white', 'MenuBar', 'none', 'units', 'pixels', 'position', [2000 400 500 300]); hold on
    
    % reward times
    plot = plotPSTH2(session, cell, rewardTimes);
    
    % obs on
    times = {osbOnTimes(isLightOn), obsOnTimes(isLightOff)};
    plot = plotPSTH2(session, cell, times, {'light on', 'light off'});
    
    % obs on to obs off
    times = {cat(1, obsOnTimes(isLightOn), obsOffTimes(~isLightOn)), ...
             cat(1, obsOnTimes(isLightOn), obsOffTimes(~isLightOn))};
    plot = plotPSTH2(session, cell, times, {'light on', 'light off'});
    
    % wisk contact
    times = {frameTimeStamps(wiskContactFrames(wiskContactFrames>0 & isLightOn)), ...
             frameTimeStamps(wiskContactFrames(wiskContactFrames>0 & ~isLightOn))};
    plot = plotPSTH2(session, cell, times, {'light on', 'light off'});
    ...
    cellAxis = plotPSTH2(session, cell, {sessionEvents}, eventName);
    ...
    cellAxis = plotPSTH2(session, cell, sessionEvents);
    ...
    cellAxis = plotPSTH2(session, cell, eventTimes, 'stepTuning');
    ...
    cellAxis = plotPSTH2(session, cell, events, 'stepTuning');
end
