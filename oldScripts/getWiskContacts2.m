function [wiskTouchSignal, wiskTouchPixels] = getWiskContacts2(vid, showTracking, frameTimeStampsWisk, obsPixPositions, obsOnTimes, obsOffTimes)
...
if showTracking
    ...
    figure('position', [10 1 750 450], 'menubar', 'none', 'color', 'black');
...
for i = 1:length(obsOnTimes)
    ...
    trialInds = find(frameTimeStampsWisk>=obsOnTimes(i) & frameTimeStampsWisk<=obsOffTimes(i) & ...
                     obsPixPositions'>=obsPixLims(1) & obsPixPositions'<=obsPixLims(2));
    if ~isempty(trialInds)
        for j = trialInds'

            % raw
            frame = 255 - rgb2gray(read(vid, j));
        ...
        %         obsMaskShifted(1:end-borderThickness, 1:end-borderThickness) = obsMask(borderThickness+1:end, borderThickness+1:end); shifted left and up
            ...
            % store results
            wiskTouchSignal(j) = sum(wiskInBorder(:)) / sum(borderMask(:));
            wiskTouchPixels{j} = find(wiskInBorder);


            % update displays
            if showTracking

                if wiskTouchSignal(j) > wiskTouchThresh
