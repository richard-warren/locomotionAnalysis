...
isStanceBins = exist('stanceBins', 'var');
...
function changeFrames(~,~)
    ...
    switch key
        ...
        % 'd': delete detection/correction
        case 100
            pause(.001);
            paused = true;
            locations.locationsRaw(frameInds(currentFrameInd), :, lastCorrectedPaw) = nan;
            locations.locationCorrections(frameInds(currentFrameInd), :, lastCorrectedPaw) = nan;
            mergeCorrectionsAndInterpolate(locations.trialIdentities(frameInds(currentFrameInd)), lastCorrectedPaw);
            updateFrame(0);
