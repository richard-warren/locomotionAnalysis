...
obsBotThickness = 15;
obsWiskThickness = 3; % 43
obsWiskYLims = [122 219]; 
...
load([getenv('OBSDATADIR') 'sessions\' session '\runAnalyzed.mat'], 'obsPositions', 'obsTimes',...
                                            'obsPixPositions', 'obsPixPositionsWisk',...
                                            'wheelPositions', 'wheelTimes',...
                                            'obsOnTimes', 'obsOffTimes',...
                                            'frameTimeStamps', 'frameTimeStampsWisk', 'webCamTimeStamps',...
                                            'touchSig', 'touchSigTimes',...
                                            'wiskTouchSignal', 'wiskContactPixels');
...
%             [timeDif, wiskFrameInd] = min(abs(frameTimeStampsWisk - frameTimeStamps(frameInds(j))));
            ...
            if ~isempty(wiskFrameInd) % timeDif < .01 % only write wisk frame if it is temporally close to run frame
                ...
                frameWisk = addObsToFrame(frameWisk, obsPixPositionsWisk(wiskFrameInd), obsWiskThickness, obsWiskYLims, 255);
