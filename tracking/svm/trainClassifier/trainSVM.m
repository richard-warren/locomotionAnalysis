function trainSVM(className)
    ...
    for i = 1:length(categories)
        ...
        temp = dir(['trainClassifier\trainingImages\' className '\' cat]); files = {temp.name}; files = files(3:end);
        load(['trainClassifier\trainingImages\' className '\' cat '\' files{1}], 'imgTemp');
        ...
        for j = 1:length(files)
            load(['trainClassifier\trainingImages\' className '\' cat '\' files{j}])
    ...
    % TRAIN CLASSIFIER
    classifier = fitcsvm(features, labels, 'KernelFunction', 'linear', 'kernelscale', 'auto', 'standardize', 1, 'boxconstraint', 1);
    subHgt = size(imgTemp,1);
    subWid = size(imgTemp,2);
    uisave ({'classifier', 'subHgt', 'subWid'}, ['trainClassifier\classifiers\classifier_' className '_' date '.mat']);
end
