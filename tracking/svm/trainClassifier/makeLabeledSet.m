...
%     startPosits = [40 70; 40 215; 140 70; 140 215];
    ...
    while imNumberInd<length(imNumbers)
        ...
        frame = read(vid,randi(vid.numberofframes));
        ...
        if ~isempty(key) && isnumeric(key)
            if key==13
                ...
                for j=1:egsPerFrame
                    ...
                    save(['trainClassifier\trainingImages\' className '\positive\img' num2str(imNumbers(imNumberInd)) '.mat'], 'imgTemp');
                    imNumberInd = imNumberInd+1;
                    
                    % create/save negative examples for every positive example
                    for j=1:negativeEgsPerEg
