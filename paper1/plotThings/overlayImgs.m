function overlay = overlayImgs(imgs, varargin)
...
% initializations
if exist('varargin', 'var'); for i = 1:2:length(varargin); s.(varargin{i}) = varargin{i+1}; end; end  % set colorspace if color is specified as a string
if ischar(s.colors); s.colors = eval([s.colors '(size(imgs,3))']); end % set colorspace if color is specified as a string

% color each image
imgsColored = nan(size(imgs,1), size(imgs,2), 3, size(imgs,3));
for i = 1:size(imgs,3)
    img = imgs(:,:,i);
