...
touchTypes = {'fore_ventral', 'fore_dorsal', 'hind_ventral_low'};  % {'fore_ventral', 'fore_dorsal', 'hind_ventral_low', 'hind_dorsal'};
...
verticalSpacing = 0; % number of pixels separating rows
...
for i = 1:length(touchTypes)
    ...
    t = any(touchesPerPaw==touchInds(i), 2);  % touches for features i
...
%                 if t(rowInds(j)); frame(end-2:end,:) = 255; end  % underline images where contact is detected
        ...
        fprintf('WARNING! Can''t find contact %s\n', touchTypes{i});
    end
end
