...
models = table(cell(nrows,1), cell(nrows,1), cell(nrows,1), zeros(nrows, 1), nan(nrows,1), 'RowNames', rowNames, ...
    'VariableNames', {'groups_pre', 'model_pre', 'model', 'dev_pre', 'dev'});
...
X = X_full(:, colBins);
modelTimeOnly = fitModel(X, y, s.lambdas);
models{'full', 'model_pre'}{1} = modelTimeOnly;
models{'full', 'dev_pre'} = cvdeviance(X, y, modelTimeOnly, 'holdout', true, 'bestLambdaOnly', true);
...
for i = 1:length(groups)
    ...
    % train pre model
    trainPreModel = ~isnan(groupInfo{groups{i}, 'residuals'}{1});
    
    if trainPreModel
        preGroups = split(groupInfo{groups{i}, 'residuals'}{1}, ' ');
        models{groups{i}, 'groups_pre'}{1} = preGroups;
        members = predictorInfo.Properties.RowNames(ismember(predictorInfo.group, preGroups));  % members of group
        preColBins = ismember(colNames, [members; 'time']);
        X = X_full(:, preColBins);
        model = fitModel(X, y, lambda_min);
        models{groups{i}, 'model_pre'}{1} = model;
        models{groups{i}, 'dev_pre'} = cvdeviance(X, y, model, ...
            'holdout', true, 'bestLambdaOnly', true, 'timeBins', timeBins);
    else
        preColBins = strcmp(colNames, 'time');
        models{groups{i}, 'model_pre'}{1} = modelTimeOnly;
        X = X_full(:, preColBins);
        models{groups{i}, 'dev_pre'} = cvdeviance(X, y, modelTimeOnly, ...
            'holdout', true, 'bestLambdaOnly', true, 'timeBins', timeBins);
    end
    ...
    dev = cvdeviance(X, y, model, 'holdout', true, 'bestLambdaOnly', true, 'timeBins', timeBins);
