function data = getKinematicData2(sessions)
    ...
    for j = 1:length(obsOnTimes)
        ...
        for k = 1:4
            ...
            for m = 1:size(locations,2)
                trialLocations(:,m,k) = interp1(trialTimeStamps, locations(trialBins,m,k), trialTimeStampsInterp);
            end
        end
        ...
        if ~any(isnan(trialLocations(:))) && ~any(isExcluded(trialBins)) && trialVels(j)>=minVel % !!! this is a hack // should check that velocity criteria is met AND that the locations have in fact been analyzed for the session
            ...
            for k = 1:4
                ...
                modifiedWheelVels(k) = interp1(wheelTimes, vel, stepTimes(1));
                ...
                for m = 1:controlSteps
                    ...
                    controlWheelVels(m,k) = interp1(wheelTimes, vel, stepTimes(1));
                end
            end
