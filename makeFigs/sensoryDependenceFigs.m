...
%% entropy (use average across mice...)

heatmaps = plotDecisionHeatmaps(flat, 'condition', 'sensoryCondition', 'levels', vars.sensoryCondition.levels, 'outcome', 'isModPawLengthened', ...
    'deltaMin', 0, 'successOnly', false, 'modPawOnlySwing', m.modPawOnlySwing, 'lightOffOnly', m.lightOffOnly, ...
    'avgMice', true, 'plotMice', true, 'colors', sensColors, 'xLims', [-20 15], 'normalize', 'col');
close gcf

ent = cellfun(@entropy, heatmaps);

figure('position', [400 472.00 382.00 328.00], 'color', 'white', 'menubar', 'none');
barFancy(ent, 'levelNames', {vars.sensoryCondition.levelNames}, 'ylabel', 'landing position entropy (bits)', ...
...
%% model accuracies
% rng(1)
plotModelAccuracies(flat, m.predictors(1:5), 'isModPawLengthened', ...
    'condition', 'sensoryCondition', 'levels', vars.sensoryCondition.levels, 'weightClasses', true, ...
    'deltaMin', .5, 'successOnly', false, 'modPawOnlySwing', m.modPawOnlySwing, 'lightOffOnly', m.lightOffOnly, ...
