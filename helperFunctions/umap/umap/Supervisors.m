...
classdef Supervisors < handle
    ...
    methods(Static)        
        ...
        function [plots, btns, btnLbls]=Plot(data, lbls, lblMap, fromD, ...
    ...
    methods
        ...
        function [names, lbls, clrs]=getQfTrained(this, data)
            ...
            isSigId = cnts_>=Supervisors.MIN_FREQUENCY;
            ...
            for i=1:N_
                ...
                if id>0
                    ...
                    if isSigId(i)
                        ...
                        try
                            clrs(sig_idx,:)=str2num(value)/256; %#ok<ST2NM>
                        catch
                            clrs(sig_idx,:)=[.1 .1 .1];
                        end
                    else
        ...
        function [lbls, names, clrs, next]=addUnsupervisedClusters(...
            ...
            if any(un)
                if isempty(this.unsupervisedClues)
                    ...
                    clues(clues<0)=0;
                    u=unique(clues);
                    cnts_=UmapUtil.discreteCount(clues, u)';
                    nClues=length(cnts_);
                    for i=1:nClues
                        clue=u(i);
                        if clue>0
    ...
    methods
        ...
        function qft=qfMatch(this, priorQf, visible, pu, scenario, ...
            ...
            qft=QfTable(qf, clrs, [], get(0, 'currentFig'), visible, this.args);
            ...
            function select(qf, isTeacher, qfIdx)
                [name, lbl, tLbl, ~]=QfHiDM.GetIds2(qf, isTeacher, qfIdx);
                fprintf('teacher=%d, tId=%d, name="%s"\n',...
                    isTeacher, tLbl, name);
                if ~isempty(this.btns)
                    this.btns.get( find(this.btnLbls==lbl, 1)-1).doClick
                end
                if ~isempty(this.args.parameter_names) && this.args.roi_table>=2
                    if isTeacher
                        d=tUnreducedData(tLbls==lbl,:);
                    else
                        d=sUnreducedData(sLbls==lbl,:);
                    end
                    try
                        needToMake=isempty(this.roiTable) ...
                            || ~ishandle(this.roiTable.table.table.fig);
                    catch
                        needToMake=true;
                    end
                    if needToMake
                        this.roiTable=Kld.Table(d, ...
                            this.args.parameter_names, ...
                            this.args.roi_scales, qft.fig, name, 'south', 'Dimension', 'UMAP', ...
                            false, [], false);
                        drawnow;
                        Gui.Locate(this.roiTable.table.table.fig, ...
                            qft.fig, 'south++');
                        set(this.roiTable.table.table.fig, 'visible', 'on');
                    else
                        this.roiTable.refresh(d, name);
                    end                    
                end
            end
        end
