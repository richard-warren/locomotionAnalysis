...
classdef QfTable < handle
    ...
    methods
        function this=QfTable(qf, tClrs, props, priorFig, visible, externalArgs)
            if nargin<6
                externalArgs=[];
                if nargin<5
                    visible=true;
            ...
            this.tClrs=tClrs;
            this.qf=qf;
            PROP_COL_W='qftColumnWidthsV2';
            PROP_COL_ORD='qftColumnOrderV1';
            PROP_SORT='qftRowOrderV1';
            path=BasicMap.Path;
            if visible
                pu=PopUp('Preparing mass+distance similarity table', 'north west+', ...
            ...
            this.tb=tb_;
            figName=['Mass+distance similarity  ' ...
            ...
            try
                ...
                if ~isempty(this.qf.falsePosNegs)
                    ...
                    QfHiDM.BackgroundReadings(true, tb_);
            ...
            function select(h, e)
                ...
                rowIdx=st.jtable.getSelectedRow;
                if rowIdx>=0
                    try
                        ...
                        try
                            feval(this.fncSelect, this.qf, isTeacher, qfIdx);
                        catch ex
                            ex.getReport
                            if isTeacher
                                name=this.qf.tNames{qfIdx};
                            else
                                name=this.qf.sNames{qfIdx};
                            end
                            fprintf('rowIdx=%d, teacher=%d, name="%s"\n',...);
                                rowIdx, isTeacher, name);
                        end
                    catch ex
                        ex.getReport
                    end
        ...
        function browseFalsePosNeg(this)
            ...
            busy=Gui.ShowBusy(this.fig, ...
                Gui.YellowH2('Computing false pos/neg'),...
                'wayneMoore1.png', .95, false);
            [fig_,that]=FalsePositiveNegative.Plot([0 1], ...
                this.qf.falsePosNegFile, [], 2, ...
                this.externalArgs.class_descriptions);
        ...
        function ok=doHistQF(this, visible)
            ...
            if ishandle(this.qHistFig)
                ...
                figName=['Mass+distance similarity (QFMatch) ' ...
