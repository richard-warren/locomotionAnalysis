...
classdef TablePicker <handle
    ...
    methods(Access=private)
        ...
        function p=defineArgs(this)
            ...
            addParameter(p,'is_xy_selections', false, @logical);
            addParameter(p,'force_selections', false, @logical);
            ...
            addParameter(p,'default_row_order', [], @(x)isnumeric(x) ...
                && all(x>=0));
            addParameter(p, 'selection_background', [], @(x)isnumeric(x)...
                && length(x)==3 && all(x>=0) && all(x<=1));
            addParameter(p, 'selection_foreground', [], @(x)isnumeric(x)...
                && length(x)==3 && all(x>=0) && all(x<=1));
            ...
            function ok=validateCallback(txt, x)
                ...
                if isequal('function_handle', class(x))
                    ...
                    try
                        if ~strcmpi(txt, 'refresh')
                            ok=feval(x, this, -1, -1, {});
                        end
                        ok=true;
                    catch ex
                        ex.getReport
                        warning(['%s_callback signature is ok=pick('...
                        'this, data_rows_idxs, row_identifiers)'], txt);
                    end
                else
                    ok=isempty(x);
                end
        ...
        function [actual, prop, dflt]=specifyFolder(this, chooseNow)
            ...
            dflt=fullfile(File.Home, 'Documents', 'run_umap');
